...
p.stdin.close()
result = out.decode('latin1') if out else err.decode('latin1')
...
def write_movelog(mdata, err, pos, code):
    try:
        ...
        err = '(systemerror:01) возникла ошибка при записи файла логов. Попробуйте перезапустить программу'
        error_msg(err)
...
def read_movelog():
    try:
        path = resource_path('movelog')
        with open(path,  encoding="utf-8") as bobr:
            data = bobr.read()
            ind = None
            mdata = []
        ...
        err = '(systemerror:02) возникла ошибка при чтении файла логов. Попробуйте перезапустить программу'
        error_msg(err)
